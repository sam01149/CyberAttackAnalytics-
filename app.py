...
if uploaded_file:
    ...
    clusters = km.fit_predict(filtered_prep)
    ...
    X_2d = pca.fit_transform(filtered_prep)
    ...
    X_nb = filtered_prep.drop('Attack Type', axis=1)
    y_nb = filtered_prep['Attack Type']
    ...
    sns.heatmap(cfmat, annot=True, cmap="Blues", xticklabels=label_encoders['Attack Type'].classes_, yticklabels=label_encoders['Attack Type'].classes_)
    plt.xlabel("Pred1iksi")
    plt.ylabel("Aktual")
    plt.title("Confusion Matrix")
    st.pyplot(f)

    # Classification report
    st.text(classification_report(y_test, y_pred, target_names=label_encoders['Attack Type'].classes_))
    ...
    with st.form("PrediksiNB"):
        ...
        fl = st.number_input("Financial Loss (in Million $)", min_value=0.5, max_value=100.0, step=1.0)
        n_affected = st.number_input("Number of Affected Users", min_value=420, max_value=999999, step=1000)
        ...
        rt_hr = st.number_input("Incident Resolution Time (in Hours)", min_value=1, max_value=72, step=1)
    ...
    if sbmt:
        Xbaru = pd.DataFrame([[
            ...
            year,
            0, # Attack Type (dummy, tak dipakai)
            label_encoders['Target Industry'].transform([target_ind])[0],
            scaler.transform([[fl, n_affected, rt_hr]])[0][0], # Financial Loss scaled
            scaler.transform([[fl, n_affected, rt_hr]])[0][1], # Number affected scaled
            label_encoders['Attack Source'].transform([attack_source])[0],
            label_encoders['Security Vulnerability Type'].transform([vuln])[0],
            label_encoders['Defense Mechanism Used'].transform([defense])[0],
            scaler.transform([[fl, n_affected, rt_hr]])[0][2], # Resolution Time scaled
        ]], columns=['Country','Year','Attack Type','Target Industry', 'Financial Loss (in Million $)', 'Number of Affected Users', 
                     'Attack Source', 'Security Vulnerability Type', 'Defense Mechanism Used', 'Incident Resolution Time (in Hours)'])

        # attack type pred
        y_pred_new = nb.predict(Xbaru.drop('Attack Type', axis=1))
        pred_label = label_encoders['Attack Type'].inverse_transform(y_pred_new)[0]

        # cluster mana
        cluster_pred = km.predict(Xbaru)
        st.success(f"Prediksi Attack Type: **{pred_label}** (masuk cluster {cluster_pred[0]})")
    ...
    # --- Download data prediksi (jika diperlukan bisa expand)

else:
    st.info("Silakan upload file dataset untuk mulai.")
